...
fn parse_args() -> ArgMatches<'static> {
    let matches = App::new("Cabinet")
        ...
        .subcommand(
            SubCommand::with_name("blog")
                .about("Blog template generate, for my quick writing.")
                .version("0.1")
                .author("Lucas Jin")
                .arg(Arg::with_name("title").help("title of blog").index(1))
                .arg(
                    Arg::with_name("date")
                        .short("d")
        ...
        .subcommand(
            SubCommand::with_name("code")
                ...
                .version("0.1")
                .author("Lucas Jin")
                .arg(Arg::with_name("name").help("name of project.").index(1)),
        )
        .subcommand(
...
fn main() {
    println!(
        "{} - {}",
        "Cabinet".yellow().bold(),
        "The Ultimate Tool Box".yellow().bold()
    );
    println!(
        "written by {} with {}.",
        "Lucas Jin".green().bold(),
        "Rust".red().bold()
    );
    println!(
        "{} \nwelcome folk and star!",
        "https://github.com/jinfagang/cabinet"
    );
